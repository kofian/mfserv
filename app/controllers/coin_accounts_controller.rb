class CoinAccountsController < ApplicationController
  ...
  # POST /accounts.json
  def create
    @account = CoinAccount.new(account_params)
    @account.id = SecureRandom.random_number(999999999999)
    #@account.customer_id = current_user.customer.id
    @account.balance = 0
    @account.date_opened = Time.now.to_date
  ...
  private
    ...
    def set_account
      @account = CoinAccount.find(params[:id])
    end
